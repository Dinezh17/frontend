...
const EmployeeGap: React.FC = () => {
    ...
    const calculateGap = (required: number, actual: number | null): number => {
        return actual !== null ? required - actual : 0;
    };
                    ...
                    >
                        ...
                        {expandedEmployeeId === employee.id && (
                            <div style={{ marginTop: "16px" }}>
                                <h4 style={{ fontSize: "16px", fontWeight: "bold", marginBottom: "8px" }}>Competencies</h4>
                                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                                    ...
                                    <tbody>
                                        {getCompetenciesForEmployee(employee.id).map((comp) => {
                                            const gap = calculateGap(comp.required_score, comp.actual_score);
                                            return (
                                                <tr key={comp.id}>
                                                    ...
                                                    <td style={{ padding: "8px", border: "1px solid #ddd", textAlign: "center", color: gap > 0 ? "red" : "green" }}>{gap}</td>
                                                </tr>
                                            );
                                        })}
                                    </tbody>
